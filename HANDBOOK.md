...
其中 `-xelatex` 选择 XeLaTeX，`-cd` 在源文件所在目录编译，
`-outdir=build` 将 PDF、日志和辅助文件写入 `build/`。

### 为什么输出到 `build/`

这不是 LaTeX 默认行为，而是当前 `Makefile` 的设置。直接运行
`xelatex resume-cn.tex` 时，LaTeX 默认把 PDF 和辅助文件放在当前目录。

如果想改成其他目录，把 `Makefile` 中所有 `-outdir=build` 改成例如
`-outdir=output`；如果删除该参数，文件就会生成在 `latex/` 根目录。
Overleaf 不依赖本地 `Makefile`，会自行管理编译产物，因此不要上传 `build/`。

### Overleaf 编译

1. 创建空白项目并按“第 8 节”的清单上传文件；
2. 将主文档设为 `resume-cn.tex` 或 `resume-en.tex`；
3. 将编译器设为 XeLaTeX；
4. 点击 Recompile。

## 2. 文件职责

- `resume-cn.tex`：中文章节、经历和技能正文；
- `resume-en.tex`：英文章节、经历和技能正文；
- `chicv.cls`：字体、页边距、通用页眉、Logo、章节、列表、技能表格和私有字段组件；
- `basic_info/basic_info.json`：姓名、标题、头像配置和动态页眉字段；
- `assets/`：头像、小 Logo 和右上角大 Logo；
- `fonts/`：Font Awesome 图标字体和 Palatino 粗体字体。
...
`basic_info/basic_info.json` 中修改。中英文主文件已经调用：
...
中英文正文分别位于 `resume-cn.tex` 和 `resume-en.tex`。章节格式为：
...
- `assets/entry-logo.png`：经历标题前的小 Logo；
- `assets/header-logo.png`：右上角大 Logo。
...
章节标题字号在 `chicv.cls` 的 `\cvsection` 中设置：
...
页面边距由 `chicv.cls` 中的 `geometry` 设置：
...
`fonts/Palatino Linotype.ttf`；图标使用 `fonts/FontAwesome6.otf`。中文字体按
`fonts/SimSun.ttf`、系统 `Songti SC`、Overleaf 自带 `FandolSong` 的顺序回退。
替换字体时需要同步修改 `chicv.cls` 中的字体文件名或字体名称。
...
```bash
cp basic_info/basic_info.example.json basic_info/basic_info.json
```
